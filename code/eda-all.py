# def eda(sentence, synonyms_dict, kelas_kata_dict, pronouns_category, category_to_word, alpha_wr, alpha_wd, alpha_wi):
...
#     # Step 1: Ambil hanya indeks yang punya sinonim
#     valid_synonym_indices = [i for i, word in enumerate(words) if get_sundanese_synonyms(word, synonyms_dict)]

#     if not valid_synonym_indices:
#         return [sentence]

#     num_to_replace = max(1, int(alpha_wr * len(valid_synonym_indices)))
#     used_synonyms_map = {}
#     remaining_indices = valid_synonym_indices[:]
...
#         print("\n=== Augmentasi Sinonim Baru ===")
#         print(f"Kalimat awal: {sentence}")
#         print(f"Indeks sinonim yang bisa dimodifikasi: {remaining_indices}")
...
#         print(f"[EDA] Kalimat hasil sinonim: {new_sentence}")

#         # Step 2: Lanjutkan dengan pronouns_replacement()
#         pronoun_sentences, _ = pronouns_replacement(new_words, kelas_kata_dict, alpha_wr * len(words), {})
#         for p_sent in pronoun_sentences:
#             sentence_text = ' '.join(p_sent)
#             if sentence_text not in augmented_sentences:
#                 augmented_sentences.append(sentence_text)
#                 print(f"[EDA] Kalimat hasil pronouns_replacement: {sentence_text}")

#         # Step 3: Lanjutkan dengan number_replacement()
#         number_sentences = number_replacement(new_words, alpha_wr * len(words))
#         for n_sent in number_sentences:
#             sentence_text = n_sent
#             if sentence_text not in augmented_sentences:
#                 augmented_sentences.append(sentence_text)
#                 print(f"[EDA] Kalimat hasil number_replacement: {sentence_text}")
...
#         if alpha_wr > 0:    
#             synonym_sentences = eda(sentence, synonyms_dict, kelas_kata, pronouns_category, category_to_word, alpha_wr=alpha_wr, alpha_wd=0, alpha_wi=0)
...
#         if alpha_wi > 0:
#             insertion_sentences = eda(sentence, synonyms_dict, kelas_kata, pronouns_category, category_to_word, alpha_wr=0, p_wd=0, alpha_wi=alpha_wi)
